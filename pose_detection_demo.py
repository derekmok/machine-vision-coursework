...
# Get all video files and select 4 with different push-up counts
...
def extract_frame(video_path, position=0.25):
    """Extract a single frame from a video at the specified position (0-1)."""
...
# %%
# Create PoseLandmarker for IMAGE mode
...
def draw_landmarks(frame, pose_landmarks):
    ...
    landmarks = pose_landmarks[0]  # First detected pose
...
for idx, (video_name, frame) in enumerate(frames.items()):
    ...
    # Extract push-up count from filename
