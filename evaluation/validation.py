"""Validation on unseen data with visualizations."""
...
def plot_validation_density_maps(results: Optional[list[ValidationResult]]) -> None:
    ...
    for idx, (ax, result) in enumerate(zip(axes, results)):
        ...
        peak_height_threshold = np.max(density_map) * 0.1  # At least 10% of max height
        peaks, _ = find_peaks(
            density_map, 
...
def plot_peak_frames(results: Optional[list[ValidationResult]], validation_dir: str = "validation-data") -> None:
    ...
    for result in results:
        ...
        # Use prominence and height thresholds to filter significant peaks
        peak_height_threshold = np.max(density_map) * 0.1  # At least 10% of max height
        peaks, properties = find_peaks(
            density_map, 
            height=peak_height_threshold,
            prominence=0.01,  # Minimum prominence to filter noise
            distance=5  # Minimum distance between peaks (frames)
        )
        ...
        # Create subplot grid - up to 5 columns
        ...
        for i, peak_idx in enumerate(peaks):
            ...
            ax.set_title(f'Peak {i+1}: Frame {peak_idx}\nDensity: {peak_height:.3f}',
        ...
        # Save each video's peaks to a separate file
